...
static void glnvg__getUniforms(GLNVGshader* shader)
{
	...
	shader->loc[GLNVG_LOC_TEXSIZE] = glGetUniformLocation(shader->prog, "texSize");
...
static int glnvg__renderCreate(void* uptr)
{
	...
	static const char* fillVertShader =
		...
		"	ftcoord = tcoord;\n"
		"	fpos    = vertex/texSize;\n"
		"	vec3 tVertex  = mvpMatrix * vec3(vertex, 1.0);\n"
		"	gl_Position = vec4(2.0*tVertex.x/viewSize.x - 1.0, 1.0 - 2.0*tVertex.y/viewSize.y, 0, 1);\n"
		"}\n";
	...
	static const char* fillFragShader = 
		...
		"	} else if (type == 1) {\n"
		"		gl_FragColor = texture2D(tex, fpos);\n"
		"	} else {"
		"		discard;\n"
		"	}\n"
		"}\n";
...
static void glnvg__setUniforms(GLNVGcontext* gl, int uniformOffset, int image)
{
	...
	glUniform2fv(gl->shader.loc[GLNVG_LOC_TEXSIZE], 1, frag->extent);
...
static void glnvg__doScissor(GLNVGcontext* gl, GLNVGcall* call)
{
	int x =  call->x >= 0 ? call->x : 0;
	int y =  call->y >= 0 ? call->y : 0;
	int w =  call->w > 0 ? call->w : gl->view[0];
	int h =  call->h > 0 ? call->h : gl->view[1];
...
static void glnvg__renderFlush(void* uptr)
{
	...
	if (gl->ncalls > 0) {
		...
		for (i = 0; i < gl->ncalls; i++) {
			...
			else if (call->type == GLNVG_SCISSOR) {
				glnvg__doScissor(gl, call);
			}
		}
...
static void glnvg__scissor(void* uptr, int x, int y, int w, int h)
{
	GLNVGcontext* gl = (GLNVGcontext*)uptr;
	GLNVGcall* call = glnvg__allocCall(gl);
	float height = gl->view[1];
...
#endif
{
	...
	params.renderScissor = glnvg__scissor;
